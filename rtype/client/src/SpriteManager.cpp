...
sprites["missile"] = LoadTexture((basePath + "BigBullet.png").c_str());
sprites["background"] = LoadTexture((basePath + "background.png").c_str());
sprites["powerup"] = LoadTexture((basePath + "powerup.png").c_str());
// ... (avec vérifications d'erreur)
    ...
    if (sprites["background"].id == 0) std::cerr << "ERREUR: Chargement échoué pour " << (basePath + "background.png") << std::endl;
    ...
    if (sprites["powerup"].id == 0) std::cerr << "ERREUR: Chargement échoué pour " << (basePath + "powerup.png") << std::endl;
    // ... vérifier les autres ...
