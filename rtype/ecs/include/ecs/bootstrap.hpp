...
namespace Components {
    ...
    struct Damage_t {
        ...
        // ✅ NOUVEAU : Constructeur avec paramètre
        explicit Damage_t(int dmg) : _damage(dmg) {}
          operator int() const { return _damage; };
    ...
    struct  Healthpoints_t {
        ...
        Healthpoints_t() = default;
        
        // ✅ NOUVEAU : Constructeur avec paramètre
        explicit Healthpoints_t(int hp) : _hp(hp) {}
        operator int() const { return _hp; };
    ...
    struct PlayerStats {
        int xp {0};
        int level {1};
        int score {0}; // <-- AJOUT : score autoritatif du joueur
        PlayerStats() = default;
        PlayerStats(int xp_, int level_, int score_ = 0) : xp(xp_), level(level_), score(score_) {}
    };
    struct BulletOwner {
        uint32_t ownerNetworkId{0};
        BulletOwner() = default;
        explicit BulletOwner(uint32_t id) : ownerNetworkId(id) {}
    };
    ...
    struct Drawable {
        ...
        float height = 30.0f; // Taille par défaut
           explicit Drawable(uint8_t type) 
        : serverEntityType(type), width(30.0f), height(30.0f) {}
    };
