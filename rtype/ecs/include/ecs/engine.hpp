...
typedef struct {
    int hp;
    entity id;                        // ID ECS local
    uint32_t networkId;               // ✅ NOUVEAU : ID réseau
    int damage;
    Components::Velocity vel;
    Components::Position pos;
    Components::PlayerStats stat;
    Components::Controllable control;
    Components::Healthpoints_t health;
} playerInfo;

typedef struct {
    entity id;                        // ID ECS local
    uint32_t networkId;               // ✅ NOUVEAU
    Components::Velocity vel;
    Components::Position pos;
    Components::Enemystype enemy;
} EnemyInfo;
...
public:
   ...
   void add_score(uint32_t playerNetworkId, int delta);
    int get_player_score(uint32_t playerNetworkId) const;
    ...
    std::vector<PlayerScoreInfo> collect_all_player_scores() const;
    ...
    std::set<entity> get_player_entities() const { return player_entity_set; } // MODIFIÉ : Utilise la variable renommée
        ...
        std::vector<OrbsInfo> getOrbs(void);
        std::vector<EnemyInfo> getEnemysInfo(void);
        std::vector<BulletInfo> getBulletsInfo(void);
        std::vector<playerInfo> getPlayersInfo(void);
};
